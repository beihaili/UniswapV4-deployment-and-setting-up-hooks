...
describe("Mycontract", function() {
    ...
    before(async function() {
        ...
        token0 = await depolyContract("Token0");
        token1 = await depolyContract("Token1");
        ...
        //depoly poolManager
        poolManager = await depolyContract("PoolManager", 88888888888);
        ...
        MyLiquidityProvider = await depolyContract("MyLiquidityProvider", poolManager.address);
        //console.log("MyContract deployed to:", MyLiquidityProvider.address);
        //await isdepolyed(MyLiquidityProvider.address);
    });

    it("ERC20 token swap test", async function() {
    //initial poolManager
    ...
    approveERC20(token1,MyLiquidityProvider.address,ethers.utils.parseUnits("21000000", 18))

    // // check my allowance
    // checkAllowance(token0, wallet.address, poolManager.address);
    // checkAllowance(token1, wallet.address, poolManager.address);
    // checkAllowance(token0, wallet.address, MyLiquidityProvider.address);
    // checkAllowance(token1, wallet.address, MyLiquidityProvider.address);
    ...
    //swap
    let sqrtpricelimit=ethers.BigNumber.from('79228162514264337593543950336')//price = 10000
    ...
    //await approveERC20(token0,poolManagerAddress,ethers.utils.parseUnits("21000000", 18))
    //await approveERC20(token1,poolManagerAddress,ethers.utils.parseUnits("21000000", 18))
    
    
  });
//   it ("ERC20 and ETH token swap test", async function() {
//     let sqrtPriceX96 = "792281625142643375935439503360"// price = 100 token1/token0
//     let poolKey: PoolKey = {
...
//     //approve ERC20 token to MyLiquidityProvider
//     approveERC20(token0,MyLiquidityProvider.address,ethers.utils.parseUnits("21000000", 18))
//     //approve ETH to MyLiquidityProvider
//     //approveERC20(token1,MyLiquidityProvider.address,ethers.utils.parseUnits("21000000", 18))
//     //add liquidity
//     let modifyPositionParams = {
//         tickLower: 45000, // lower price 90
...
//         //swap
//         let sqrtpricelimit=ethers.BigNumber.from('792281625142643375935439503360')//price = 10000
//         let amountswap = ethers.BigNumber.from('123456789101112134')
//         console.log("amountswap:",amountswap.toString())
...
// //         hooks:     "0x0000000000000000000000000000000000000000",
// //     };
// //     let token0befordonate = await getERC20Balance(token0,wallet.address);
// //     console.log("donate")
// //     let amountswap = ethers.BigNumber.from('123456789101112134')
// //     await donate (MyLiquidityProvider, poolKey, amountswap,  amountswap);

// //     let token0afterdonate = await getERC20Balance(token0,wallet.address);
// //     console.log("token0 change:",token0afterdonate.sub(token0befordonate).toString())
// //     });
});